...
async function saveBattleHistory() {
  try {
    await authFetch('/api/addBattleHistory', {
      method: 'POST',
...
async function startNormalBattle() {
  ...
  const environmentResponse = await authFetch('/api/getEnvironment')
...
async function doBattle(card) {
  ...
  try {
    const enemyResponse = await authFetch('/api/enemyCard')
    if (!enemyResponse) return
    const enemyCard = await enemyResponse.json()

    const battleResponse = await authFetch('/api/battle', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        playerCard: card,
...
<template>
  <div class="battle-scene">
    <div v-if="!result && !loading && !choosingCard" class="battle-panel">
      ...
      <div class="battle-modes">
        ...
        <div class="battle-mode battle-mode--random" @click="startRandomBattle">
          ...
          <p class="battle-mode__desc">Dos cartas aleatorias luchan entre si en una batalla</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="choosingCard" class="card-picker">
      <h3 v-if="battleEnvironment" class="card-picker__title">Escenario de hoy</h3>
      <div v-if="battleEnvironment" class="scenario-banner" :style="{ '--ec': CARD_TYPES[battleEnvironment.affectedType].color }">
        <div class="scenario-banner__info">
          <span class="scenario-banner__name">{{ battleEnvironment.name }}</span>
          <span class="scenario-banner__desc">{{ battleEnvironment.description }}</span>
        </div>
        <div class="scenario-banner__effect">
          <span class="scenario-banner__type">{{ battleEnvironment.affectedType }}</span>
        </div>
      </div>
    ...
    <div v-if="result" class="battle-arena">
      <div ref="playerCardRef" class="battle-card-wrapper" :class="{ 'battle-card--loser': battleFinished && !playerWon }">
        ...
        <Card :card="result.playerCard" :interactive="false" />
      </div>
      ...
      <div ref="enemyCardRef" class="battle-card-wrapper" :class="{ 'battle-card--loser': battleFinished && playerWon }">
        ...
        <Card :card="result.enemyCard" :interactive="false" />
      </div>
    </div>
